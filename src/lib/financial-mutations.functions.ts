...
export const mutateFinancialData = createServerFn({ method: "POST" }).middleware([requireSupabaseAuth])
  .validator((data: Mutation) => { if (!data?.empresaId) throw new Error("Empresa obrigatória."); return data; })
  .handler(async ({ context, data }) => {
    const userId = String(context.userId); await assertEmpresaAccess(userId, data.empresaId);
    switch (data.action) {
      case "saveSaldos": {
